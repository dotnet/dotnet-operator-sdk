...
{
    public Task<ReconciliationResult<V1DemoEntity>> ReconcileAsync(
        V1DemoEntity entity,
...
{
    public Task<ReconciliationResult<V1DemoEntity>> FinalizeAsync(
        V1DemoEntity entity,
        CancellationToken cancellationToken)
    {
...
A recommended reading path from here to a production-ready operator:

1. **Learn the building blocks** — [Custom Entities](./building-blocks/entities), [Controllers](./building-blocks/controllers), [Finalizers](./building-blocks/finalizer), and (if you need admission control) [Webhooks](./building-blocks/webhooks). The [Examples](./examples) show each of them in a runnable project.
2. **Secure it** — declare the permissions your operator needs with [RBAC](./rbac).
3. **Configure it** — tune reconciliation, queuing, and leader election in [Advanced Configuration](./configuration).
4. **Test it** — write [unit and integration tests](./testing/unit-tests) for your controllers and finalizers.
5. **Observe it** — wire up [logging, tracing, and metrics](./observability/logging).
6. **Ship it** — generate manifests and deploy with the [CLI](./cli) and the [Deployment guide](./deployment).
