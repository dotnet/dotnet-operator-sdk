...
When enabled, every managed entity is checked for the components its configuration requires. If anything
is missing, host startup aborts with an `InvalidRegistrationException` that lists the gaps — turning a
silent misconfiguration (an operator that starts but processes nothing) into an immediate, actionable
error.
...
It also flags entities that have a finalizer but no controller, since finalizers run as part of
reconciliation and need a controller to execute.

:::note
Components are recognized by deriving from the SDK base types (`ResourceWatcher<TEntity>` /
`EntityQueueBackgroundService<TEntity>`). Keep validation disabled if you replace these with
implementations that do not derive from them.
:::
...
most one pending entry per namespace/name key, preserve the earliest scheduled execution time, and do
not expose cancellation/removal as part of `ITimedEntityQueue<TEntity>`.
