...
It is set via `OperatorSettings.ReconcileStrategy` and defaults to `ByGenerationId`.
...
|---|---|---|
| `metadata.generation` | The API server | Only when `.spec` changes |
...
builder.Services
    .AddKubernetesOperator(settings =>
    {
        settings.ReconcileStrategy = ReconcileStrategy.ByGenerationId; // default
    });
```
...
**Consequences:**
- Status updates do **not** trigger reconciliation.
- Label or annotation changes do **not** trigger reconciliation.
...
:::tip When to use `ByGenerationId`
