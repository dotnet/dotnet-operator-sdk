...
Namespaces that are only known at runtime (for example a `WATCH_NAMESPACE` environment variable) cannot be resolved
statically. Use the `--rbac-scope` option to decide the scope explicitly instead:
...
| Value        | Result                                                                    |
| ------------ | ------------------------------------------------------------------------- |
| `auto`       | Default. Derives the scope from the operator configuration in the source. |
| `cluster`    | Always generates `ClusterRole` and `ClusterRoleBinding`.                  |
| `namespaced` | Always generates `Role` and `RoleBinding`.                                |

With `--rbac-scope namespaced`, the namespace of the generated `Role` and `RoleBinding` is taken from `--namespace`,
otherwise from a statically discovered namespace, otherwise it falls back to `<operator-name>-system`. In the fallback
case the namespace resource is generated as well, so the kustomization can be relocated with
`kustomize edit set namespace`.

For MSBuild based generation the same values are available through the `KubeOpsRbacScope` and `KubeOpsNamespace`
properties, see [build customization](./build-customization).

The initial namespaced generation support deploys the operator into the watched namespace. If the CLI `--namespace`
option is supplied, it must match a statically configured `OperatorSettings.Namespace`. Cluster-scoped entities and non-resource URL rules
...
4. **Testing**:
   - Test against a cluster **using the generated service account**, not your admin kubeconfig —
     locally everything works because admin bypasses RBAC (see the note above), so missing rules
     only surface in-cluster
   - A quick check without deploying: `kubectl auth can-i watch <resource> --as=system:serviceaccount:<ns>:<operator-sa>`
   - Watch the operator logs for `Forbidden` errors after every RBAC change (see
     [Troubleshooting — 403 Forbidden](./troubleshooting#403-forbidden--rbac-errors))
...
1. **Missing Permissions**:
   ...
   - Leader election fails (the lease permissions are part of the [default RBAC rules](#default-rbac-rules); see also [Leader Election — Troubleshooting](./configuration/leader-election#troubleshooting))
