...
By default, KubeOps automatically attaches finalizers to entities during reconciliation. This behavior can be configured through `OperatorSettings`. See [Advanced Configuration](../configuration/finalizer-lifecycle) for details on controlling automatic finalizer attachment.
...
### 2. Assuming Finalizers Run in Parallel

KubeOps executes finalizers **sequentially, one at a time, in the order of the entity's finalizer
list**: only the first finalizer is invoked; once it succeeds and is removed, the resulting
`MODIFIED` event triggers the next one. Don't build finalizers that wait on each other — the
later one simply won't run until the earlier one has completed. `DeletedAsync` on the controller
fires only after **all** finalizers are removed.

### 3. Blocking Deletion on an Unavailable Dependency

A finalizer that requires an external system to respond will keep the resource in `Terminating`
for as long as that system is down. Decide per dependency what "cleanup impossible" means: fail
and retry (default), or log and succeed when the external resource is already gone / the cleanup
is not critical. Treat "not found" responses from external systems as success — that is the
idempotency case from Best Practice 1.
