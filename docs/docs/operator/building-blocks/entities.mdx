...
- `[CustomResourceDefinitionFileName]`: Overrides the generated CRD file name, including its extension. If omitted,
  KubeOps keeps using the CRD's fully qualified name with dots replaced by underscores and appends the selected
  output format extension. Apply the same file name to every version of a multi-version CRD.
...
- `[DefaultValue]`: Adds an OpenAPI default value. Set `Json = true` on a string argument to represent structured
  values such as objects or arrays instead of a literal string
...
- `[Format]`: Overrides the inferred OpenAPI format, for example `[Format("uri")]` or `[Format("email")]`.
  Use `[Format(null)]` to suppress an inferred format such as `int32`
...
[CustomResourceDefinitionFileName("demoentities.demo.kubeops.dev.yaml")]
