...
| `labelSelectorPath` | No | JSON path to the label selector string, e.g. `.status.selector` |

### Example — spec-only entity
...
### Example — entity with both spec and status
...
The generated CRD will contain:
...
subresources:
  scale:
    ...
    labelSelectorPath: .status.selector  # only if provided
```

:::note
The scale subresource is independent of the status subresource. You can use `[ScaleSubresource]` on an entity that has no `Status` property, and the status subresource will not be added. Conversely, a `Status` property always activates the status subresource regardless of whether `[ScaleSubresource]` is present.
:::
