...
:::tip[Solution files]
Instead of a `.csproj` file, you can pass a `.sln` or `.slnx` solution file. The CLI will compile all projects in the solution and search them for custom resources. Use `--project` (regex) and `--target-framework` to narrow the scope.
:::
