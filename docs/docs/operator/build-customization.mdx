...
<PropertyGroup>
    ...
    <!-- Optional space-separated resource groups (defaults to all) -->
    <KubeOpsConfigResources>crds deployment</KubeOpsConfigResources>
...
```xml
<PropertyGroup>
    <!-- Disable ANSI colored output (automatically detected in CI environments) -->
    <KubeOpsNoAnsi>true</KubeOpsNoAnsi>
</PropertyGroup>
```

KubeOps tries to automatically detect common CI/CD environments and then disables ANSI colored output:
...
- TeamCity
- Bitbucket Pipelines
- Generic CI environments (CI=true)

You can manually override this behavior by setting `<KubeOpsNoAnsi>true</KubeOpsNoAnsi>` or `<KubeOpsNoAnsi>false</KubeOpsNoAnsi>`.
...
1. **BaseConfig**:
   ...
   - Sets default values for:
     ...
     - `KubeOpsConfigResources`: Optional space-separated resource groups passed to `--resources`
...
```
KubeOps CLI is not installed or not available. Install it as a .NET tool (e.g. 'dotnet tool install KubeOps.Cli' or 'dotnet tool install --global KubeOps.Cli'), or override <KubeOpsCli> in your project.
```

To install the CLI tool (local tool is recommended):
...
If you need to override how the CLI is invoked (for example, when installing to a custom tool path), you can set the `KubeOpsCli` MSBuild property:
...
4. **CI/CD Integration**:
   - ANSI colors are automatically disabled in CI environments
   - Override with `KubeOpsNoAnsi` if needed
...
For local development, you may want to not automatically generate resources in Debug mode and have them installed in your cluster. You can combine `GenerateOperatorResources` with the [CRD Installer utility](./configuration/crd-installer) for this purpose.
