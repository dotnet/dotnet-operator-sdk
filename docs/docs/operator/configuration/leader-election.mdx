...
- When `ScopeChanged` is raised, the watched entities are re-listed (applying the configured
  namespace, label selector, and field selector) and run through the regular event path, so changes that happened
  while another instance was (or nobody was) responsible are picked up; the deduplication cache
  skips entities that are already reconciled in their current state.
