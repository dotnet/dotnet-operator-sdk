...
- When `ScopeChanged` is raised, this entity type's deduplication cache is dropped and the watched
  ...
  and run through the regular event path, so changes that happened while another instance was (or
  nobody was) responsible are picked up. Clearing the cache first guarantees that an entity handed
  back to this instance is reconciled even when its `generation`/`resourceVersion` is unchanged
  since the previous ownership term — the takeover reconcile would otherwise be suppressed as a
  duplicate. Reconcilers must therefore be idempotent, since every in-scope entity is reconciled
