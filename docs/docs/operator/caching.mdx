...
1. **`MODIFIED` / `ADDED` event**: the watcher reads the cached generation and compares it with
   ...
   Label and annotation changes never increment `generation` (they are `.metadata` changes).
   Status updates do not increment `generation` when the CRD has a
   [status subresource](https://kubernetes.io/docs/tasks/extend-kubernetes/custom-resources/custom-resource-definitions/#status-subresource) enabled.
...
2. **`DELETED` event**: same as `ByGeneration` — the cache entry is removed and the event is
   always forwarded.
