...
KubeOps provides an `EventPublisher` to create and update events for your custom resources. The publisher is available through dependency injection in your [controllers](./building-blocks/controllers) and [finalizers](./building-blocks/finalizer).

:::note
Publishing events requires write permissions on the `events` resource — these are part of the
[default RBAC rules](./rbac#default-rbac-rules) KubeOps generates. To change how the event
objects themselves are constructed (naming, labels, additional metadata), see
[Customizing Resource Generation](./configuration/generation-customization#customizing-event-generation).
:::
...
1. **Event spam from the reconcile loop**: Publishing an event on *every* `ReconcileAsync` call
   floods etcd — reconciliations run on every spec change, requeue, and retry. Publish events for
   **state transitions** (created, became ready, failed), not for no-op reconciliations. KubeOps
   deduplicates identical events for you: the same entity + reason + message + type maps to the
   same event name, so the publisher increments `Count` instead of creating a new object (see
   [how the default factory works](./configuration/generation-customization#customizing-event-generation)).

2. **Dynamic values defeat deduplication**: Putting timestamps, counters, or exception `.ToString()`
   output into the message makes every event unique — the `Count` aggregation stops working and
   each occurrence becomes a separate object. Keep `reason` and `message` stable and put variable
   details into logs instead.

3. **Wrong event type**: `EventType.Warning` should mean "a human may need to act" — using it for
   routine conditions trains users to ignore warnings, while reporting real failures as `Normal`
   hides them from `kubectl get events --field-selector type=Warning`.
