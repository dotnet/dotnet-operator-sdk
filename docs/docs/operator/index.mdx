...
## How KubeOps Runs Your Operator

KubeOps implements this loop for you: it watches the Kubernetes API for changes to your resources, queues the events, and calls your controller code to reconcile them.

```mermaid
flowchart LR
    API[Kubernetes<br/>API Server] -->|watch| W[KubeOps]
    W -->|queue| C[Your Controller<br/>ReconcileAsync]
```

You implement the business logic in a controller; KubeOps takes care of watching, deduplication, queuing, and parallelism. The full event pipeline is described in [Controllers](./building-blocks/controllers#the-event-pipeline).
