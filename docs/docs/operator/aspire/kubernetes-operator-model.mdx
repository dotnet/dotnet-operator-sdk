...
For a namespaced operator, set the existing publish `Namespace` option to the same value. If the watch namespace is
only known at runtime, set `RbacScope` to `namespaced` (or `cluster` to force cluster-wide RBAC):

```csharp
publish.RbacScope = "namespaced";
```
